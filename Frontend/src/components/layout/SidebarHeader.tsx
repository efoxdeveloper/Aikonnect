...
export function SidebarHeaderBrand() { const { state } = useSidebar(); return <div data-testid="sidebar-brand-header" aria-label="AiKonnect" className={cn("group flex h-full items-center gap-2.5 bg-white px-4", state === "collapsed" && "justify-center px-0")}><BrandLogo /><div className={cn("overflow-hidden whitespace-nowrap text-[24px] font-semibold leading-none tracking-[-0.035em] text-[var(--text-primary)] transition-[width,opacity] duration-150", state === "collapsed" && "w-0 opacity-0")}><span>Ai</span><span className="text-[var(--brand)]">Konnect</span></div></div>; }
