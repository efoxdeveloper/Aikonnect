  ...
  ({ checked, onCheckedChange, className, disabled, ...props }, ref) => (
    <button
      ...
      className={cn(
        ...
        checked ? "bg-[var(--brand-accent)]" : "bg-[var(--gray-300)]",
        className,
      )}
      {...props}
    >
      <span className={cn("pointer-events-none block size-[18px] rounded-full bg-white shadow-[0_1px_3px_rgba(4,45,29,.20)] transition-transform", checked ? "translate-x-[18px]" : "translate-x-0.5")} />
    </button>
  ),
);
