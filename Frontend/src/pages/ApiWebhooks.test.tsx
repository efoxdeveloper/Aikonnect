...
function auth(permissions = ["workspace.read", "workspace.update"]): AuthContextValue {
  return {
    ...
    user: {
      ...
      memberships: [{
        id: "membership-1",
        workspace: { id: "workspace-1", name: "Acme", slug: "acme", country: "India", timezone: "Asia/Kolkata", onboardingCompletedAt: "2026-09-01T00:00:00.000Z" },
...
function renderPage(value = auth()) {
  return render(<AuthContext.Provider value={value}><MemoryRouter><ApiWebhooks /></MemoryRouter></AuthContext.Provider>);
}

describe("ApiWebhooks", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(apiRequest).mockImplementation(async (path) => String(path).includes("/webhooks") ? { items: [] } as never : { items: [key] } as never);
  });
  ...
  it("creates a key and displays its secret only in the creation result", async () => {
    vi.mocked(apiRequest).mockImplementation(async (path, options = {}) => {
      ...
      if (options.method === "POST") return { apiKey: key, secret: "sk_live_new-secret" } as never;
      return { items: [] } as never;
    });
    renderPage();
    fireEvent.change(screen.getByLabelText("Key name"), { target: { value: "Backend sync" } });
    fireEvent.click(screen.getByRole("button", { name: /generate api key/i }));
    expect(await screen.findByDisplayValue("sk_live_new-secret")).toBeInTheDocument();
    expect(screen.getByText(/will not be shown again/i)).toBeInTheDocument();
    expect(apiRequest).toHaveBeenLastCalledWith("/workspaces/workspace-1/api-keys", expect.objectContaining({ method: "POST", body: JSON.stringify({ name: "Backend sync" }) }));
  });
  ...
  it("revokes an active key after confirmation", async () => {
    ...
    await waitFor(() => expect(apiRequest).toHaveBeenLastCalledWith("/workspaces/workspace-1/api-keys/key-1", expect.objectContaining({ method: "DELETE" })));
    expect(await screen.findByText("Revoked")).toBeInTheDocument();
  });

  it("creates an HTTPS webhook and displays its signing secret once", async () => {
    const webhook = { id: "webhook-1", name: "Production app", url: "https://example.com/webhooks", events: ["message.received"], active: true, lastDeliveredAt: null, createdAt: "2026-09-01T00:00:00.000Z" };
    vi.mocked(apiRequest).mockImplementation(async (path, options = {}) => {
      if (String(path).includes("/webhooks") && options.method === "POST") return { webhook, secret: "whsec_new-secret" } as never;
      if (String(path).includes("/webhooks")) return { items: [] } as never;
      return { items: [] } as never;
    });
    renderPage();
    ...
    expect(await screen.findByDisplayValue("whsec_new-secret")).toBeInTheDocument();
  ...
  it("keeps key management disabled for users without workspace update permission", async () => {
    ...
    expect(await screen.findByText(/only workspace administrators/i)).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /generate api key/i })).toBeDisabled();
