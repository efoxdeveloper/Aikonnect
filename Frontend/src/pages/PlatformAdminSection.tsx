...
function Billing({ data }: { data: { subscriptions: { configured: boolean; message: string }; wallet: { currency: string; balance: string }; wallets?: Array<{ tenantId: string; tenantName: string; tenantSlug: string; workspaceCount: number; currency: string; balance: string }>; sharedWhatsAppBilling: { totalAccounts: number; allocatedAccounts: number; unallocatedAccounts: number; statuses: CountMap }; workspaceCount: number } }) {
  const auth = useContext(AuthContext);
