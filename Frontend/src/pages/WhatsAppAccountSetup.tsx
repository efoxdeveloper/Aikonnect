import { ArrowLeftIcon as ArrowLeft, CheckIcon as Check, ExternalLinkIcon as ExternalLink, MessageSquareIcon as MessageSquare, ShieldCheckIcon as ShieldCheck } from "@animateicons/react/lucide";
    ...
    <div className="mx-auto max-w-[1050px] px-5 py-7 sm:px-8 sm:py-9">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
        <section className="rounded-md border border-[var(--border-soft)] bg-white p-6 shadow-[0_3px_12px_rgba(30,40,55,.045)] sm:p-8">
          ...
          <button type="button" disabled className="mt-7 flex h-11 w-full cursor-not-allowed items-center justify-center rounded-md bg-[var(--brand)] px-4 text-sm font-medium text-white opacity-55">
            Continue with Meta
            <ExternalLink ref={externalIcon.ref} size={15} duration={0.6} className="ml-2" aria-hidden="true" />
          </button>
          <p className="mt-2.5 text-center">The connection becomes available after the Meta app credentials and callback endpoint are configured.</p>
        </section>
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-md border border-[#d7ebec] bg-[var(--brand-soft)] p-5">
            ...
            <p className="mt-2">App ID, App Secret, Configuration ID and a webhook verification token are required next.</p>
          </section>
        </aside>
      </div>
    </div>
