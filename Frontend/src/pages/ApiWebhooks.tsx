...
export function ApiWebhooks() {
  ...
  return <div data-testid="api-webhooks-page" className="flex h-full min-h-0 flex-col overflow-hidden bg-[var(--page-background)]">
    ...
    <nav aria-label="API and webhook sections" data-testid="api-webhooks-tabs" className="flex flex-none overflow-x-auto border-b border-[var(--border-soft)] bg-white px-5 sm:px-8">
      <div role="tablist" className="mx-auto flex w-full max-w-[1100px] gap-5">
        {tabs.map(({ id, label, icon: Icon }) => <button key={id} id={`${id}-tab`} type="button" role="tab" aria-selected={activeTab === id} aria-controls={`${id}-panel`} onClick={() => { setActiveTab(id); setError(null); }} className={`relative flex h-12 shrink-0 items-center gap-2 border-b-2 px-1 text-xs font-medium transition-colors ${activeTab === id ? "border-[var(--brand)] text-[var(--brand)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]"}`}><Icon size={15} />{label}</button>)}
      </div>
    </nav>
    <div data-testid="api-webhooks-scroll-region" className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto max-w-[1100px] space-y-5 px-5 py-5 sm:px-8 sm:py-7">
      {!canManage && activeTab !== "docs" && <div className="flex items-start gap-3 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800"><ShieldCheck size={16} className="mt-0.5 shrink-0" />Only workspace administrators can create or revoke integration credentials. You can still read the documentation.</div>}
